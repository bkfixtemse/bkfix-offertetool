...
import { bestelSpecString } from '../calc/bestelspec';
...
export function downloadBestelbon(items: OfferItem[], klantNaam: string) {
  ...
  const headers = [
    ...
    'Bediening', 'Extra opties', 'Bestelspecificaties', 'Opmerkingen',
  ];
  const rows = items.map((it, i) => {
    ...
    return [
      ...
      bediening, extra, bestelSpecString(it), it.opmerkingen,
    ];
  });
  const title = [`BKfix Bestelbon  |  ${klantNaam || 'Offerte'}  |  ${date}`];
  const ws = XLSX.utils.aoa_to_sheet([title, [], headers, ...rows]);
  ws['!cols'] = [4, 14, 18, 7, 10, 14, 18, 14, 14, 14, 18, 16, 14, 12, 22, 16, 14, 20, 34, 44, 30].map((wch) => ({ wch }));
